...
import Jump from '@successtar/react-reveal/Jump'
  ...
  return (
    <header className='fixed top-0 left-0 bg-white w-full h-fit z-[9999]'>
      <div className='container h-16 lg:h-[5.25rem] flex items-center justify-between'>
        <Link className='block w-fit hover:text-orange duration-300' href="/">
          <Logo />
        </Link>
        <button
          onClick={handleClick}
        ...
        <Jump left cascade when={active} appear>
          <nav className={classNames('mobile-nav-menu fixed left-0 w-screen h-screen bg-white gap-y-8 flex flex-col justify-center p-10 md:p-16 duration-300', {
            'bottom-0': active,
            '-bottom-[100vh]': !active,
          })}>
            {headerMenu.length > 0 &&
              headerMenu.map((link: any, index: any) => (
                <Link
                  key={`link-${index}`}
                  className={classNames('font-semibold text-3xl md:text-5xl btn-nav', {
                  'active': router.asPath.includes(link.url)})}
                  href={link.url}
